...
export class WebGl implements ICelestialRenderer {
  ...
  init(height: number, width: number): void {
    ...
    const skyUrls = [
      'galaxy_rit.png', 'galaxy_lft.png', 'galaxy_top.png',
  ...
  updateNavWaypointDuration(index: number, durationSec: number): void {
    if (this.navRoute.waypoints[index]) {
      this.navRoute.waypoints[index].durationSec = durationSec;
    }
  }
  ...
  private async createSolarSystem(planets: any[]): Promise<void> {
    ...
    const starRings: RingConfig[] = Array.isArray((this.star.config as any).rings)
      ? (this.star.config as any).rings
      : (Array.isArray(starData.rings) ? starData.rings : []);

    await this.starFactory.attachRings(this.star, starRings);
  ...
  animate(): void {
    ...
    for (const ring of this.starFactory.keplerianRings) {
